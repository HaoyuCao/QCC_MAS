...
from qcc.agents import create_agent
from qcc.prompts import PromptManager
from qcc.scenarios.base import BaseScenario, CaseInput, ScenarioResult
from qcc.specialist_pool import (
    SpecialistAgent,
    SpecialistPool,
    RouterAgent,
    ConsistencyAgent,
    query_specialist_for_ppe,
)

if TYPE_CHECKING:
    from qcc.config import ModelConfig
    from qcc.agents.chat_agent import AgentWrapper
...
class ProgressiveQCCScenario(BaseScenario):
    ...
    def __init__(
        ...
        specialist_pool: "SpecialistPool | None" = None,
        ...
        Args:
            ...
            free_exploration_turns: K — free exploration turns before first planning (default 3)
            specialist_pool: Specialist candidate pool for EDPM routing.
            enable_specialist_routing: When False, uses generic Planner (bypass router).
            num_specialists: Number of specialists Router selects (default 3).
                Each proposes 1 PPE; Doctor receives num_specialists PPE targets.
            consistency_agent: Pluggable agent for EBVM Criterion 3
                (Agents Consistent Approve). Defaults to always-CONSISTENT.
        """
        ...
        self.specialist_pool = specialist_pool or SpecialistPool.default()
    ...
    def run(self, case_input: CaseInput) -> ScenarioResult:
        ...
        planner = None  # Created lazily after specialist routing
        ...
        while current_turn <= self.max_turns:
            ...
            if self._should_planner_intervene(current_turn, has_significant_finding):
                # --- Specialist routing + PPE collection ---
                if self.enable_specialist_routing:
                    # Step 1: Router Agent → top-N specialists based on S_obs
                    router = RouterAgent(
                        pool=self.specialist_pool, top_k=self.num_specialists
                    )
                    top_specialists, routing_meta = router.route(dialogue_history)

                    trace.append({
                        "role_id": "router",
                        "content": json.dumps({
                            "selected": [s.specialist_id for s in top_specialists],
                            "routing_meta": routing_meta,
                        }, ensure_ascii=False),
                        "duration": 0.0,
                    })

                    # Step 2: Each specialist independently proposes 1 PPE
                    ppe_list: list[dict[str, Any]] = []
                    for spec in top_specialists:
                        ppe_result = query_specialist_for_ppe(
                            specialist=spec,
                            observed_evidence=dialogue_history,
                        )
                        if ppe_result is not None:
                            ppe_list.append(ppe_result)
                            trace.append({
                                "role_id": f"specialist_{spec.specialist_id}",
                                "content": json.dumps(ppe_result, ensure_ascii=False),
                                "duration": 0.0,
                            })

                    # Step 3: Map specialists' PPEs → PlannerState (for Doctor prompt compat)
                    planner_state = PlannerState()
                    for i, spec in enumerate(top_specialists):
                        priority = "HIGH" if i == 0 else ("MEDIUM" if i < 3 else "LOW")
                        planner_state.system_directions.append(SystemDirection(
                            system=spec.name,
                            priority=priority,
                            basis=spec.focus,
                        ))
                    for ppe in ppe_list:
                        planner_state.priority_evidence.append(PriorityEvidence(
                            evidence_type=ppe.get("ppe", ""),
                            reason=ppe.get("rationale", ""),
                            associated_system=ppe.get("specialist", ""),
                        ))
                    planner_state.internal_reasoning = (
                        f"Router → {[s.specialist_id for s in top_specialists]}. "
                        f"Assessment: {routing_meta.get('overall_assessment', '')}"
                    )
                    planner_state.add_planning_record(current_turn, "multi-specialist PPE collection")

                elif planner is None:
                    # Fallback: generic Planner agent (no specialist routing)
    ...
    def _run_progressive_verification_pipeline(
        ...
        is_consistent, consistency_reason = self.consistency_agent.check(
            diagnosis=answer,
            confidence=confidence,
            verifier_assessment=v_response,
        )
        trace.append({
            ...
            "content": json.dumps({
                ...
                "diagnostician_confidence": confidence,
            }, ensure_ascii=False),
            "duration": 0.0,
        })

        # --- Handle Planner feedback from Verifier ---------------------------
        if planner_feedback and planner_state is not None and planner is not None:
